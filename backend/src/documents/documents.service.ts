 ...
 *
 * `responses.userId` and `versions.uploadedById` point at Neon Auth's
 * `neon_auth."user"` table, which isn't a Prisma relation (see
 * NeonAuthUsersRepository) — `enrichDocument()` below batch-fetches and
 * attaches `.user`/`.uploadedBy` onto the raw query result below.
 */
...
export class DocumentsService {
  ...
  /**
   * Attaches `.user` on each response and `.uploadedBy` on each version of a
   * document queried with DOCUMENT_FULL_INCLUDE — those used to be Prisma
   * relation includes; userId/uploadedById now point at Neon Auth's table,
   * which isn't a Prisma relation (see NeonAuthUsersRepository).
   */
  private async enrichDocument<
    T extends {
      responses: { userId: string }[];
      versions: { uploadedById: string }[];
    },
  >(doc: T) {
    const ids = [
      ...new Set([
        ...doc.responses.map((r) => r.userId),
        ...doc.versions.map((v) => v.uploadedById),
      ]),
    ];
    ...
    return {
      ...
      versions: doc.versions.map((v) => ({
        ...v,
        uploadedBy: userMap.get(v.uploadedById) ?? { id: v.uploadedById, name: "Unknown" },
      })),
    };
  }
  ...
  async findByProject(
    ...
    const enriched = await Promise.all(data.map((doc) => this.enrichDocument(doc)));
  ...
  async setSignatureFields(
    ...
    return updated ? this.enrichDocument(updated) : null;
  }
  ...
  async uploadVersion(
    ...
    return this.enrichDocument(result);
  }
  ...
  async getVersions(id: string, orgId: string) {
    ...
    const uploaders = await this.neonAuthUsers.findMany(uploaderIds);
  ...
  async getAccessTokens(documentId: string, orgId: string) {
    ...
    const users = await this.neonAuthUsers.findMany(userIds);
  ...
  async generateCertificate(id: string, orgId: string) {
    ...
    // Enrich audit events + signer responses with user names (one batch query)
    const userIds = [
      ...new Set([
        ...auditEvents.filter((e) => e.userId).map((e) => e.userId!),
        ...doc.responses.map((r) => r.userId),
      ]),
    ];
    const users = await this.neonAuthUsers.findMany(userIds);
    ...
    for (const resp of signerResponses) {
      ...
      const name = respUser?.name || "Unknown";
