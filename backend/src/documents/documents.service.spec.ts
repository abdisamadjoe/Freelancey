...
const mockPrisma = {
  project: {
    findFirst: vi.fn(() => Promise.resolve({ id: PROJECT_ID, organizationId: ORG })),
  },
  document: {
    findFirst: vi.fn(() => Promise.resolve(makeDoc())),
    findUnique: vi.fn(() => Promise.resolve(makeDoc())),
    create: vi.fn((args: { data: Record<string, unknown>; include?: unknown }) =>
      Promise.resolve({ id: DOC_ID, ...args.data, signatureFields: [], responses: [], file: {}, signedFile: null }),
    ),
    update: vi.fn((args: { where: Record<string, unknown>; data: Record<string, unknown>; include?: unknown }) =>
...
const mockNeonAuthUsers = {
  findUnique: vi.fn(() => Promise.resolve({ id: CLIENT_USER, name: "Test Client" })),
  findMany: vi.fn(() => Promise.resolve([])),
};
...
const mockNotifications = {
  notifyDocumentUploaded: vi.fn(() => undefined),
  notifyDocumentResponded: vi.fn(() => undefined),
  notifyDocumentSigningTurn: vi.fn(() => undefined),
};
...
function resetMocks() {
  ...
  mockNeonAuthUsers.findUnique.mockClear();
