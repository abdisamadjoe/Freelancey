...
function makeSetupService() {
  return {
    getSetupStatus: vi.fn(() => Promise.resolve({ completed: false })),
    markSetupComplete: vi.fn(() => Promise.resolve({ completed: true })),
  };
}

function makeSettingsService() {
  return {
    updateSettings: vi.fn(() => Promise.resolve({ emailProvider: null })),
