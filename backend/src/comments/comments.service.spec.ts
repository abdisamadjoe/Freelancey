...
const mockPrisma = {
  projectUpdate: {
    findFirst: vi.fn(() => Promise.resolve({ projectId: PROJECT_ID })),
  },
  task: {
    findFirst: vi.fn(() => Promise.resolve({ projectId: PROJECT_ID })),
  },
  projectClient: {
    findFirst: vi.fn(() => Promise.resolve({ projectId: PROJECT_ID, userId: USER })),
  },
  comment: {
    ...
    delete: vi.fn(() => Promise.resolve({ id: COMMENT_ID })),
  },
};
...
function clearAllMocks() {
  ...
  mockNeonAuthUsers.findMany.mockClear();
}
...
describe("CommentsService", () => {
  ...
  beforeEach(() => {
    ...
    mockNeonAuthUsers.findMany.mockReturnValue(Promise.resolve([]));
  ...
  describe("create", () => {
    describe("success paths", () => {
      ...
      it("creates a comment linked to a task when target type is 'task'", async () => {
        ...
        const callArgs = (mockPrisma.comment.create as ReturnType<typeof vi.fn>).mock.calls[0][0] as PrismaArgs;
  ...
  describe("findByTarget", () => {
    describe("success paths", () => {
      ...
      it("enriches comments with author name from user lookup", async () => {
        ...
        mockNeonAuthUsers.findMany.mockReturnValue(
      ...
      it("deduplicates authorId lookups when multiple comments share the same author", async () => {
        ...
        // findMany should be called once with a deduplicated set of IDs
        expect(mockNeonAuthUsers.findMany).toHaveBeenCalledTimes(1);
        const inIds = (mockNeonAuthUsers.findMany as ReturnType<typeof vi.fn>).mock.calls[0][0] as string[];
        expect(inIds).toHaveLength(1);
        expect(inIds[0]).toBe(USER);
      });

      it("passes an empty id list to the user lookup when there are no comments", async () => {
        mockPrisma.comment.findMany.mockReturnValue(Promise.resolve([]));
        mockPrisma.comment.count.mockReturnValue(Promise.resolve(0));

        await service.findByTarget("update", UPDATE_ID, ORG, USER, "owner", 1, 50);

        // NeonAuthUsersRepository.findMany short-circuits on an empty array
        // before it would ever touch the database.
        expect(mockNeonAuthUsers.findMany).toHaveBeenCalledWith([]);
      });
      ...
      it("queries comments filtered by updateId when target type is 'update'", async () => {
        ...
        const findManyArgs = (mockPrisma.comment.findMany as ReturnType<typeof vi.fn>).mock.calls[0][0] as PrismaArgs;
...
import { describe as describeCtrl, expect as expectCtrl, it as itCtrl, beforeEach as beforeEachCtrl } from "vitest";
...
describeCtrl("CommentsController", () => {
  ...
  beforeEachCtrl(() => {
    commentsService = {
      create: vi.fn(() => Promise.resolve({ id: COMMENT_ID, content: "x" })),
      findByTarget: vi.fn(() => Promise.resolve({ data: [], meta: {} })),
