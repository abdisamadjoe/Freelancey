...
const mockPrisma = {
  subscriptionPlan: {
    findMany: vi.fn(() => Promise.resolve([FREE_PLAN, PRO_PLAN, LIFETIME_PLAN])),
    findUnique: vi.fn(() => Promise.resolve(FREE_PLAN)),
  },
  subscription: {
    findUnique: vi.fn(() =>
      Promise.resolve({
    ...
    create: vi.fn(() => Promise.resolve({ id: "sub-new" })),
    update: vi.fn(() => Promise.resolve({ id: "sub-1" })),
  ...
  file: {
    aggregate: vi.fn(() => Promise.resolve({ _sum: { sizeBytes: 5242880 } })),
  },
  member: {
    count: vi.fn(() => Promise.resolve(1)),
  },
  organization: {
    findUnique: vi.fn(() => Promise.resolve({ id: "org-1", name: "Test Org" })),
  },
};

const mockStripeService = {
  resolveKey: vi.fn((suffix: string) => suffix === "SECRET_KEY" ? "sk_test_123" : ""),
  stripe: {
    customers: {
    ...
    checkout: {
      sessions: {
        create: vi.fn(() => Promise.resolve({ url: "https://checkout.stripe.com/pay/test" })),
      },
    },
    billingPortal: {
      sessions: {
        create: vi.fn(() => Promise.resolve({ url: "https://billing.stripe.com/session/test" })),
      },
    },
  },
};
